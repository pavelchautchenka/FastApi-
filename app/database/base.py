...
class Manager:
    ...
    async  def get_event(cls):
        ...
        async with db_conn.session as session:
            result = await session.execute(select(cls).where(cls.meeting_time>=current_time ))
        return result.scalars().all()

    @classmethod
    async def get_event_with_id(cls,id):
        current_time = datetime.now()
        async with db_conn.session as session:
            result = await session.execute(select(cls).where(cls.id == id and cls.meeting_time >= current_time))
